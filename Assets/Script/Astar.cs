        ...
        while (true)
        {
            ...
            if (Physics.BoxCast(PointsList[pointIndex], start.transform.localScale * 0.5f , vec, out hit, Quaternion.LookRotation(vec), distance))
            {
                ...
                else
                {
                    ...
                    if (bb.transform.localScale.y < (start.transform.localScale.y / 4))
                    {
                        float length = Vector3.Distance(PointsList[pointIndex], hit.point);
                        length -= 0.5f;
                        Vector3 jumpStart = PointsList[pointIndex] + ((hit.point - PointsList[pointIndex]).normalized * length);
                        jumpStart = new Vector3(jumpStart.x, start.transform.position.y, jumpStart.z);
                        PointsList.Add(jumpStart);
                        ++pointIndex;
                        length = Vector3.Distance(PointsList[pointIndex], hit.point);
                        length += bb.localScale.x * 2;
                        Vector3 jumpEnd = PointsList[pointIndex] + ((hit.point - PointsList[pointIndex]).normalized * length);
                        jumpEnd = new Vector3(jumpEnd.x, start.transform.position.y, jumpEnd.z);
                        PointsList.Add(jumpEnd);
                        ++pointIndex;
                        jumpIndex = pointIndex;
                        break;
                    }
                    else
                    {
                        ...
                        PointsList.Add(corners[cornerIndex]);
                        ++pointIndex;
                    }
                }
            }
